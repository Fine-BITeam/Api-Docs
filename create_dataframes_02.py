...
def get_json_from_API(url, api_name, load_from_disk=True):
    if load_from_disk and os.path.exists(f'{apis_response_path}/{api_name}.txt'):
        response_text = open(f'{apis_response_path}/{api_name}.txt', 'r', encoding="utf8").read()
    else:
        payload = {}
        headers = {'Api-Key': 'dNtr6w4juJTZ9vWhwNmD3bKrAf6TdAsM'}
        response = requests.request("GET", url, headers=headers, data=payload)
        response_text = response.text
        with open(f'{apis_response_path}/{api_name}.txt', 'w', encoding="utf8") as f:
...
for api_name, url in urls_type1.items():
    try:
        ...
        df_response_list.to_csv(os.path.join(dataframes_path, f'{api_name}.csv'), index=False)
...
for api_name, value in urls_type2.items():
    try:
        ...
        df_combined.to_csv(os.path.join(dataframes_path, f'{api_name}.csv'), index=False)
